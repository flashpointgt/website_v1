 ...
 * Tags: custom login, login, registration, form, login widget, widget
 * Version: 0.8.3
...
class s8_login_registration {
        ...
        elseif(isset($wp_query->query_vars[self::ep_login])) {
            ...
            switch($_GET['action']) {
                ...
                case 'rp':
                    ...
                    if($_POST['s8-login-reset'] == 'Change Password') {
                ...
                case 'forgot':
                    define('S8_LOGIN_INIT', true);
                    if($_POST['s8-login-forgot'] == 'Get New Password') {
    ...
    function template_include($template) {
        global $wp_query;
        if(!isset($wp_query->query_vars[self::ep_login])) return $template;
        ...
        if($child_theme == $parent_theme) {
            // Same theme!
            foreach($order as $temp_file) {
                if(file_exists($parent_theme.'/'.$temp_file))
                    return $parent_theme.'/'.$temp_file;
            }
        }
        else {
            // Check child then parent
            foreach($order as $temp_file) {
                if(file_exists($child_theme.'/'.$temp_file))
                    return $child_theme.'/'.$temp_file;
                elseif(file_exists($parent_theme.'/'.$temp_file))
                    return $parent_theme.'/'.$temp_file;
            }
        }
    ...
    function register_user() {
        if(!wp_verify_nonce($_POST['s8-register-nonce'], 's8-register-new-user')) die('An attempt to bypass security checks was detected! Please go back to the register form and try again.');
        // Verify our data
    ...
    function user_password_reset() {
        if(!wp_verify_nonce($_POST['s8-login-nonce'], 's8_wp_custom_login-nonce-reset')) die('An attempt to bypass security checks was detected! Please go back to the password reset and try again.');
    ...
    function user_password_reset_request() {
        if(!wp_verify_nonce($_POST['s8-login-nonce'], 's8_wp_custom_login-nonce-forgot')) die('An attempt to bypass security checks was detected! Please go back to the forgot password form and try again.');
     ...
     */
    function login_user() {
        if(!wp_verify_nonce($_POST['s8-login-nonce'], 's8_CLR-login-nonce')) die('An attempt to bypass security checks was detected! Please go back to the login form and try again.');
        // We are logging in!
        $username = sanitize_user($_POST['username']);
        if($user = get_user_by('login', $username)) {
            if(user_pass_ok($username, $_POST['pwd'])) {
                if($_POST['remember'] == 'forever') $remember = true;
                else $remember = false;
                ...
                // Do our redirect!
                if(isset($redirects[$role])) $url = $redirects[$role];
                elseif($_POST['redirect'] != '') $url = $_POST['redirect'];
                else $url = home_url('/');
                wp_redirect($url, 302);
                exit;
            }
        }
        return new WP_Error('loginmismatch', 'The username/password pair you entered is invalid!');
    }
    ...
    function no_wp_login() {
        if(defined('S8_LOGIN_INIT') && S8_LOGIN_INIT === true) return;
        $check = get_option('s8_allow_wp_register');
        if($check && $_REQUEST['action'] == 'register') return;
    ...
    function filter_email_urls($message, $key) {
        ...
        $reset_url = home_url('/'.self::ep_login.'/?action=rp&key='.$key.'&login='.rawurlencode(($user_login)));
